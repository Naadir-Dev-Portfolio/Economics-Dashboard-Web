...
BLS calendars fall back to the Federal Reserve release-date API, using the existing `FRED_API_KEY`, then its public calendar when direct BLS requests are blocked. Entries identify this as `BLS via FRED`. The API supplies announced dates for these 08:30 Eastern releases; the public calendar's US Central times are converted with daylight-saving rules. If all official endpoints fail, previously verified dates are retained and labelled cached. The last-resort seed verified on 3 September 2026 covers announced releases through December 2026; it is never extrapolated. Health reporting exposes missing coverage.
